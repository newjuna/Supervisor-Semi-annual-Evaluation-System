 ...
 */
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzmFbtKkGy5tCwSEpuku2yDEFAgKMDl5ZqGGm1hJqtQ11h63dOoKULknk_Tu-WDcvip/exec';
...
/* =========================================================
   v42 모바일 조회 안정화 패치
   - 모바일/카카오톡/팀즈 인앱 브라우저에서 Apps Script 응답이 늦을 때
     18~20초 만에 실패하던 문제 완화
   - 본인확인 조회는 60초까지 대기하고 1회 자동 재시도
   ========================================================= */
(function () {
  if (window.__v42MobileLookupPatchApplied) return;
  window.__v42MobileLookupPatchApplied = true;
  if (typeof jsonpRequest !== 'function') return;

  var previousJsonpRequest = jsonpRequest;

  function isTimeoutError(err) {
    var msg = String((err && err.message) || err || '');
    return msg.indexOf('시간이 초과') >= 0 || msg.toLowerCase().indexOf('timeout') >= 0;
  }
  ...
  jsonpRequest = async function (params, timeoutMs) {
    params = params || {};

    var mode = params.mode || '';
    var isLoginLookup = mode === 'appointmentPersonListFast' || mode === 'appointmentPersonList';
    var extendedTimeout = isLoginLookup ? Math.max(Number(timeoutMs) || 0, 60000) : Math.max(Number(timeoutMs) || 0, 45000);

    try {
      return await previousJsonpRequest(params, extendedTimeout);
    } catch (err) {
      if (!isLoginLookup || !isTimeoutError(err)) throw err;

      var msg = document.getElementById('loginResultMessage');
      if (msg) {
        msg.className = 'result v39-login-result pending';
        msg.innerHTML = '모바일 네트워크 응답이 늦어 다시 조회 중입니다. 잠시만 기다려주세요.';
      }

      await delay(700);
      return previousJsonpRequest(params, extendedTimeout);
    }
  };
})();
